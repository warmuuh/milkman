package milkman.plugin.auth.oauth.scribe;
...
/**
 * custom http client that disables hostname verification
 */
public class JDKHttpClient implements HttpClient {
    ...
    private <T> Future<T> doExecuteAsync(String userAgent, Map<String, String> headers, Verb httpVerb,
            ...
            ResponseConverter<T> converter) {
        try {
            Response response = doExecute(userAgent, headers, httpVerb, completeUrl, bodyType, bodyContents);
            @SuppressWarnings("unchecked") T t = converter == null ? (T) response : converter.convert(response);
            if (callback != null) {
    ...
    private static Map<String, String> parseHeaders(HttpURLConnection conn) {
        ...
        for (Entry<String, List<String>> headerField : conn.getHeaderFields().entrySet()) {
    ...
    private static void addBody(HttpURLConnection connection, byte[] content, boolean requiresBody) throws IOException {
        ...
        if (requiresBody || contentLength > 0) {
            OutputStream outputStream = prepareConnectionForBodyAndGetOutputStream(connection, contentLength);
            if (contentLength > 0) {
    ...
    private static void addBody(HttpURLConnection connection, MultipartPayload multipartPayload, boolean requiresBody)
        ...
        for (Entry<String, String> header : multipartPayload.getHeaders().entrySet()) {
            connection.setRequestProperty(header.getKey(), header.getValue());
        }

        if (requiresBody) {
            ByteArrayOutputStream os = MultipartUtils.getPayload(multipartPayload);
